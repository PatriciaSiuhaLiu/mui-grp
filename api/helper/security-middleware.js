...
function sqlInjectionChars(val, fieldName, url) {
...
function checkChar(data, url) {
  try{
    ...
    for(let i=0; i < keys.length; i++){
        status.push(sqlInjectionChars(newObj[keys[i]], keys[i], url));
    }  
    return status    
  }catch(e){
    console.log(e);
...
function processData(data, url, res) {  
    const status = checkChar(data, url);
    if(status.length > 0){
        let specialCharDetected = status.includes(true);
        console.log("specialCharDetected -> ", specialCharDetected)
        if(specialCharDetected){
            // throw new Error('Special Characters not allowed'); 
            res.status(400).send('Special Characters not allowed');      
        }else{
        return data;
    }
    }else{
...
module.exports = function () {
    return function (req, res, next) {    
        console.log(req.url)
        if(req.url.includes('static') || req.url.includes('login') || req.url.includes('auth')){
        console.log('skipping GET request');
        }else{
        if (req.body) req.body = processData(req.body, req.url, res);
        if (req.query) req.query = processData(req.query, req.url, res);
        if (req.params) req.params = processData(req.params, req.url, res);
        }          
        next()
    }
}